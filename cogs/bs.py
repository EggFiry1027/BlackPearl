...
from BlackPearl.bp.mycloud import SFTP
from BlackPearl.bp.livestats import LiveStats
...
class BombSquad(commands.Cog):
	...
	async def stats(self, ctx, pc: str = None, player: str = None):
		...
		async def process_stats_Req(sn: str, sts: dict, pys: dict):
			...
			if player.startswith('pb-'):
				...
				for sp in asp:
					if (player == sp[0][1:]) or (player in sp[0]):
						p_stats = sts[sp[1]]
				if pys != {}:
		...
		if pc not in servers:
			await ctx.reply(
			    f"The `party_code` **`{pc}`** is wrong or doesn't exists!")
			return
		else:
	...
	async def top(self,
		...
		async def send_top(sn: str, toppers: list, s: dict, dcr: str):
			t = f"*Toppers of  -  `{sn}`*"
			d = u"**```\n{0:^4} - {1:^15} - {2:^8}\n".format(
			    'Rank', 'Player Name', dc_names[dcr][1])
			i = 1
			for tpr in toppers:
				if i < limit:
					d += u"{0:^4} - {1:^15} - {2:^8}\n".format(
					    str(i), s[tpr[1]]['name'],
					    str(s[tpr[1]][dc_names[dcr][0]]))
					i += 1
			d += '```**'
			emd = myembed(title=t, description=d, color=get_embed_color())
		...
		if (p == None) or (not isinstance(
		    limit, int)) or (limit > 20) or discrim not in dc_names:
			await send_cmd()
			return
		servers = get_json('bs_servers')
		if p not in servers:
			await ctx.reply(
			    f"The `party_code` **`{p}`** is wrong or doesn't exists!")
			return
		...
		all_players = [(a[dc_names[discrim][0]], a['aid'])
	...
	async def end(self, ctx, party_code: str = None):
		...
		if party_code not in servers:
			await ctx.reply(
			    f"The `party_code` **`{party_code}`** is wrong or doesn't exists!"
			)
			return
		req = ctx.author.id
			...
			try:
				...
				try:
					stdin, stdout, stderr = c.exec_command(
					    f"tmux send-keys \"mgr.chatmessage('exe_cmd_dc end')\" ENTER"
					)
					c.close()
	...
	async def restart(self, ctx, party_code: str = None):
		...
		if (req in owners) or (req in admins):
			try:
				...
				try:
					stdin, stdout, stderr = c.exec_command(
					    f"tmux send-keys \"mgr.restart()\" ENTER")
					c.close()
				except Exception as e:
					await ctx.reply(f"Error:```{str(e)}```")
					print(e)
	...
	async def disconnect_client(self,
		...
		if (req in owners) or (req in admins) and (cid != None):
			try:
				...
				try:
					stdin, stdout, stderr = c.exec_command(
					    f"tmux send-keys \"mgr.kick({str(cid)}, {str(bt)})\" ENTER"
					)
					c.close()
				except Exception as e:
					await ctx.reply(f"Error:```{str(e)}```")
		...
		if (req in owners) or (req in admins) and (msg != None):
			try:
				...
				try:
					if pub:
						stdin, stdout, stderr = c.exec_command(
						    f"tmux send-keys \"mgr.chatmessage('exe_cmd_dc make public')\" ENTER"
						)
					else:
						stdin, stdout, stderr = c.exec_command(
						    f"tmux send-keys \"mgr.chatmessage('exe_cmd_dc make private')\" ENTER"
						)
					c.close()
				except Exception as e:
					await ctx.reply(f"Error:```{str(e)}```")
					print(e)
				if msg in ['public', 'private']:
	...
	async def chat_message(self,
		...
		if (req in owners) or (req in admins) and (msg != None):
			try:
				...
				try:
					stdin, stdout, stderr = c.exec_command(
					    f"tmux send-keys \"mgr.chatmessage('{str(msg)}')\" ENTER"
					)
					c.close()
				except Exception as e:
					await ctx.reply(f"Error:```{str(e)}```")
	...
	async def screen_message(self,
		...
		if (req in owners) or (req in admins) and (msg != None):
			try:
				...
				try:
					stdin, stdout, stderr = c.exec_command(
					    f"tmux send-keys \"mgr.screenmessage('{str(msg)}')\" ENTER"
					)
					c.close()
				except Exception as e:
					await ctx.reply(f"Error:```{str(e)}```")
